...
class DetailMessageState extends State<DetailMessage> {
  ...
  getChat() async {
    await context.read<DetailMessageProvider>().getChats(widget.chatRoomId!).then((value){
      setState(() {
        chats = value;
      });
    });
  ...
  Widget Emoji() {
    return Container(
      ...
      child: EmojiPicker(
        textEditingController: context.watch<DetailMessageProvider>().messageController,
        onBackspacePressed: () {
          ...
          children: [
            Expanded(
              child: Card(
                ...
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.end,
                  children: [
                    context.watch<DetailMessageProvider>().image != null ? selectedImage() : textField_emoji(),
                    context.watch<DetailMessageProvider>().image != null ? const Spacer() : Container(),
                    IconButton(
                        padding: const EdgeInsets.all(0),
                        onPressed: () async {
                          await context.read<DetailMessageProvider>().pickImage();
                          if (context.read<DetailMessageProvider>().showEmoji) {
                            context.read<DetailMessageProvider>().setShowEmoji(!context.read<DetailMessageProvider>().showEmoji);
                          }
                        },
                        icon: const Icon(
                          Icons.image,
            ...
            Card(
              ...
              child: IconButton(
                ...
                onPressed: () {
                  context.read<DetailMessageProvider>().addMessage(widget.chatRoomId!,widget.uid!,widget.token!);
                },
              ),
            ),
          ],
        ...
        children: [
          Expanded(
            child: SingleChildScrollView(
              child: TextField(
                controller: context.watch<DetailMessageProvider>().messageController,
      ...
      children: [
        ClipRRect(
          ...
          child: SizedBox(
            width: 100,
            height: 100,
            child: Image.file(context.watch<DetailMessageProvider>().image!, fit: BoxFit.cover),
          ),
        ),
        Positioned(
          right: 0,
